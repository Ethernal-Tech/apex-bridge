...
func TestCreateTx(t *testing.T) {
	...
	policyScriptMultiSig, err := wallet.NewPolicyScript(walletsKeyHashes, len(walletsKeyHashes))
	require.NoError(t, err)

	policyScriptFee, err := wallet.NewPolicyScript(walletsFeeKeyHashes, len(walletsFeeKeyHashes))
	require.NoError(t, err)

	multiSigAddr, err := policyScriptMultiSig.CreateMultiSigAddress(42)
	require.NoError(t, err)

	feeAddr, err := policyScriptFee.CreateMultiSigAddress(42)
	require.NoError(t, err)
		...
		txInputsInfos := TxInputInfos{
			MultiSig: &TxInputInfo{
				TxInputs: wallet.TxInputs{
					...
					Sum: wallet.MinUTxODefaultValue * 2,
				},
