...
type BridgeReceiver = contractbinding.IBridgeStructsReceiverWithToken
...
func (ct ConfirmedTransactionsWrapper) String() string {
	...
	for i, tx := range ct.Txs {
		...
		sb.WriteString(fmt.Sprintf("Src ChainID = %s, ", ct.ChainIDConverter.ToChainIDStr(tx.SourceChainId)))
		sb.WriteString(fmt.Sprintf("Dst ChainID = %s, ", ct.ChainIDConverter.ToChainIDStr(tx.DestinationChainId)))
		...
		for j, recv := range tx.Receivers {
			...
			if recv.AmountWrapped.Uint64() != 0 {
				sb.WriteString(fmt.Sprintf(", %d + amountWrapped: %d, tokenId: %d)", recv.Amount, recv.AmountWrapped, recv.TokenId))
			} else {
				sb.WriteString(fmt.Sprintf(", %d, tokenId: %d)", recv.Amount, recv.TokenId))
			}
		}
...
func (sbw SignedBatchWrapper) String() string {
	...
	sb.WriteString(sbw.ChainIDConverter.ToChainIDStr(sbw.DestinationChainId))
	sb.WriteString("\nraw tx = ")
