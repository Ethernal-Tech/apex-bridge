...
func TestCardanoChainObserver(t *testing.T) {
	...
	t.Run("check ErrorCh", func(t *testing.T) {
		...
		chainObserver, err := NewCardanoChainObserver(context.Background(), chainConfig, txsReceiverMock, db, indexerDB,
			brAddrManagerMock, rewardBrAddrManagerMock, hclog.NewNullLogger())
		require.NoError(t, err)
	...
	t.Run("check GetConfig", func(t *testing.T) {
		...
		chainObserver, err := NewCardanoChainObserver(context.Background(), chainConfig, txsReceiverMock, db, indexerDB, brAddrManagerMock,
			rewardBrAddrManagerMock, hclog.NewNullLogger())
		require.NoError(t, err)
		require.NotNil(t, chainObserver)
	...
	t.Run("check start stop", func(t *testing.T) {
		...
		chainObserver, err := NewCardanoChainObserver(ctx, chainConfig, txsReceiverMock, db, indexerDB,
			brAddrManagerMock, rewardBrAddrManagerMock, logger)
		require.NoError(t, err)
		require.NotNil(t, chainObserver)
