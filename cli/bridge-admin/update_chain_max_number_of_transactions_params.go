...
const (
	maxNumberOfTransactionsFlag = "max-number-of-transactions"

	maxNumberOfTransactionsFlagDesc = "max number of transactions per chain batch"
)

type updateChainMaxNumberOfTransactionsParams struct {
	chainID                 string
...
func (params *updateChainMaxNumberOfTransactionsParams) ValidateFlags() error {
	...
	if params.maxNumberOfTransactions < 1 {
		return fmt.Errorf("--%s flag not specified or less than 1", maxNumberOfTransactionsFlag)
	}
...
func (params *updateChainMaxNumberOfTransactionsParams) Execute(outputter common.OutputFormatter) (common.ICommandResult, error) {
	...
	estimatedGas, _, err := txHelper.EstimateGas(
		...
		"updateChainMaxNumberOfTransactions", chainIDInt, params.maxNumberOfTransactions)
	if err != nil {
		return nil, err
	}
	...
	tx, err := txHelper.SendTx(
		ctx, wallet, bind.TransactOpts{}, func(opts *bind.TransactOpts) (*types.Transaction, error) {
			...
			return contract.UpdateChainMaxNumberOfTransactions(opts, chainIDInt, params.maxNumberOfTransactions)
		})
	if err != nil {
		return nil, err
	}
...
func (params *updateChainMaxNumberOfTransactionsParams) RegisterFlags(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		&params.chainID,
		chainIDFlag,
		"",
	...
	cmd.Flags().Uint16Var(
		&params.maxNumberOfTransactions,
		maxNumberOfTransactionsFlag,
		0,
		maxNumberOfTransactionsFlagDesc,
	)
...
var (
	_ common.CliCommandExecutor = (*updateChainMaxNumberOfTransactionsParams)(nil)
)
