...
func NewCardanoChainObserver(
	ctx context.Context,
	config *core.ChainConfig,
	txsReceiver core.CardanoTxsReceiver, stakingDB core.CardanoTxsDB,
	indexerDB indexer.Database,
	logger hclog.Logger,
) (*CardanoChainObserverImpl, error) {
	indexerConfig, syncerConfig := loadSyncerConfigs(config)

	addressesOfInterest := make(map[string]bool, len(indexerConfig.AddressesOfInterest))
	for _, x := range indexerConfig.AddressesOfInterest {
		addressesOfInterest[x] = true
	}

	err := initOracleState(indexerDB,
		stakingDB, config.StartBlockHash, config.StartSlot, config.InitialUtxos, config.ChainID, logger)
	if err != nil {
		return nil, err
	}
	...
	indexerLogger := logger.Named("staking_block_indexer")
	blockIndexer := indexer.NewBlockIndexer(indexerConfig, confirmedBlockHandler, indexerDB, indexerLogger)
	syncer := gouroboros.NewBlockSyncer(syncerConfig, blockIndexer, logger.Named("staking_block_syncer"))
...
func (co *CardanoChainObserverImpl) Start() {
...
func (co *CardanoChainObserverImpl) GetConfig() *core.ChainConfig {
	return co.config
}
...
func loadSyncerConfigs(
	...
	addressesOfInterest := []string{
		config.StakingBridgingAddr.StakingBridgingAddr,
		config.StakingBridgingAddr.FeeAddress,
	}

	addressesOfInterest = append(addressesOfInterest, config.StakingAddresses...)
...
func initOracleState(
	db indexer.Database, stakingDB core.CardanoTxsDB,
	...
	if err := stakingDB.ClearAllTxs(chainID); err != nil {
		return err
	}
